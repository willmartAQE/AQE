/*!
 * AQE Light v3.0.0 — Atomic Quantum Engine (Free Edition)
...
class AQELight {
    ...
    _getBit(token) {
        if (!this.tokenMap.has(token)) {
            // Capped at 63 bits to stay within safe Number() cast range.
            // Beyond 63 unique tokens bits alias (Bloom-style false positives),
            // which are acceptable: candidates are validated by el.matches().
    ...
    _nodeMask(el) {
        ...
        // Stateful pseudo-classes resolvable without layout
    ...
    _selectorMask(selector) {
        ...
        while (i < subject.length) {
            ...
            if (ch === '[') {
                ...
                if (end !== -1) {
                    const token = subject.slice(i, end + 1)
                        .replace(/['"]/g, '')
                        .replace(/\s+/g, '');
